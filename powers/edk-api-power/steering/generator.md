...
```bash
py ~/.kiro/powers/repos/edk-api-power/powers/edk-api-power/scripts/api_repository_manager.py query --api "{接口名}"
```
...
- 编写 event.script 逻辑时需参考读取的 Swagger 信息和已有接口的 event.script

### ⚠️ 多版本选择规则（严格，不可跳过）

**当 `query` 返回的结果包含多条记录（即同一个接口名存在多个映射文件）时，必须暂停并让用户选择参考哪一个，禁止自动选择。**

**原因：** 同一个接口在不同 Collection 中的 event.script 逻辑可能完全不同。例如 `admin/user/onboard/applications` 在 design 场景中可能提取 `studyId`，但在 procheck 场景中不需要。自动选择错误的版本会导致生成的 script 包含不适用于当前场景的逻辑。

**操作步骤：**
1. 查询返回多条记录时，向用户展示所有版本的摘要信息：
   - 映射文件名
   - 所属集合列表
2. 询问用户选择哪个版本作为参考
3. 只读取用户选择的映射文件
4. 如果用户选择"不参考任何已有版本"，则完全基于 Swagger 定义和 api-plan 信息从零生成

**示例交互：**
```
接口 admin/user/onboard/applications 在 apiRepository 中有 3 个版本：
1. admin-user-onboard-applications.1775789482137.json（所属集合：design-add-new-item, desing-crf-add-checkbox）
2. admin-user-onboard-applications-procheck.1776069488323.json（所属集合：procheck-add-adam-package）
3. admin-user-onboard-applications.1776144479174.json（所属集合：edc-submit-data）

请选择参考哪个版本？（输入序号，或输入 0 表示不参考已有版本）
```
...
1. 确保加密服务已启动（使用 `controlPwshProcess` 启动 `node ~/.kiro/powers/repos/edk-api-power/powers/edk-api-power/scripts/encrypt-server.js`）
...
3. 按 Manager 操作逻辑判断操作类型：
   - 先查询 `py ~/.kiro/powers/repos/edk-api-power/powers/edk-api-power/scripts/api_repository_manager.py query --api "{接口名}"`
   - 如果不存在 → 执行 `add_new`
