...
检查项：
1. 孤立页面（entities/、concepts/、procedures/、rules/ 下没有被其他页面引用的页面）
2. 断链（[[X]] 链接指向的 X.md 不存在）
3. index 一致性（index.md 里有记录但文件缺失的条目）
4. 目录结构合规（每个 topic 下必须有标准七子目录）
5. 测试用例误归位（wiki/{topic}/procedures/ 下检测疑似用例文件）
...
# 可能含孤立页面的子目录（被其他页面引用才算有效）
REFERENCEABLE_SUBDIRS = ["entities", "concepts", "procedures", "rules"]

# 测试用例特征关键词（文件名层面）
TESTCASE_NAME_PATTERNS = [
    re.compile(r"用例", re.IGNORECASE),
    re.compile(r"test\s*case", re.IGNORECASE),
    re.compile(r"\bregression\b", re.IGNORECASE),
    re.compile(r"\b回归\b", re.IGNORECASE),
    re.compile(r"\bTC-\d+", re.IGNORECASE),
]
...
def check_orphan_pages(topic_dir: Path, all_links: set[str]) -> list[dict]:
    """检查孤立页面：entities/concepts/procedures/rules 下没有被其他页面引用"""
    issues = []
    for subdir_name in REFERENCEABLE_SUBDIRS:
        subdir = topic_dir / subdir_name
        if not subdir.exists():
            continue

        for page_file in subdir.glob("*.md"):
            page_name = page_file.stem
            if page_name not in all_links:
                issues.append({
                    "type": "orphan_page",
                    "subdir": subdir_name,
                    "file": str(page_file.relative_to(topic_dir)),
                    "name": page_name,
                    "message": f"{subdir_name}/'{page_name}' 没有被其他页面引用",
                })
    return issues


def check_subdir_compliance(topic_dir: Path) -> list[dict]:
    """检查 topic 子目录是否合规：必须有标准七子目录、不能有多余目录"""
    issues = []

    actual_subdirs = {d.name for d in topic_dir.iterdir() if d.is_dir()}
    expected = set(STANDARD_SUBDIRS)

    missing = expected - actual_subdirs
    extra = actual_subdirs - expected

    for sub in missing:
        issues.append({
            "type": "missing_subdir",
            "subdir": sub,
            "message": f"缺失标准子目录: {sub}/",
        })
    for sub in extra:
        issues.append({
            "type": "extra_subdir",
            "subdir": sub,
            "message": f"出现非标准子目录: {sub}/（标准目录见 wiki-structure.md）",
        })
    return issues


def check_testcase_misplaced(topic_dir: Path) -> list[dict]:
    """检查 wiki/{topic}/procedures/ 等下是否有疑似测试用例文件"""
    issues = []
    suspect_dirs = ["procedures", "concepts", "entities", "rules"]
    for subdir_name in suspect_dirs:
        subdir = topic_dir / subdir_name
        if not subdir.exists():
            continue
        for page_file in subdir.glob("*.md"):
            name = page_file.stem
            for pat in TESTCASE_NAME_PATTERNS:
                if pat.search(name):
                    issues.append({
                        "type": "testcase_misplaced",
                        "subdir": subdir_name,
                        "file": str(page_file.relative_to(topic_dir)),
                        "name": name,
                        "message": f"'{name}' 命名疑似测试用例，应路由到 testcases/ 由 edk-baize-tests 管理",
                    })
                    break
    return issues
...
def lint_topic(wiki_root: Path, topic_name: str) -> dict:
    ...
    # 执行检查
    issues = []
    issues.extend(check_subdir_compliance(topic_dir))
    ...
    issues.extend(check_testcase_misplaced(topic_dir))
    ...
    return {
        ...
        "summary": {
            ...
            "missing_subdirs": len([i for i in issues if i["type"] == "missing_subdir"]),
            "extra_subdirs": len([i for i in issues if i["type"] == "extra_subdir"]),
            "testcase_misplaced": len([i for i in issues if i["type"] == "testcase_misplaced"]),
        },
    }
