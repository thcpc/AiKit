...
5. 测试用例关联判断：
   ...
   - 用户拒绝 → 跳过
   - ⛔ 禁止默认关联，必须等待用户确认
6. 读取 `.baize-schema.md` 中的语言设置，按以下规则生成输出文件：
   ⛔ **语言判断规则**：必须完整匹配字段值，禁止子字符串匹配（"中英文" ≠ "英文"）：
   - 字段值 = `中文` → 单份中文文件
   - 字段值 = `英文` → 单份英文文件
   - 字段值 = `中英文` → 生成两份文件：
     - `queries/{日期}-{标题}_cn.md`（中文内容）
     - `queries/{日期}-{标题}_en.md`（英文内容，为中文版的翻译）
     - ⛔ 两份文件内容完全对应，英文为中文的翻译
7. 根据 `assets/query-template.md` 生成结果
...
6. 读取 `.baize-schema.md` 中的语言设置，按以下规则生成输出文件：
   ⛔ **语言判断规则**：必须完整匹配字段值，禁止子字符串匹配（"中英文" ≠ "英文"）：
   - 字段值 = `中文` → 单份中文文件，使用标准路径和命名
   - 字段值 = `英文` → 单份英文文件，使用标准路径和命名
   - 字段值 = `中英文` → 生成两份文件，命名规则：
     - 中文版：`{原文件名}_cn.md`
     - 英文版：`{原文件名}_en.md`（为中文版的翻译）
     - ⛔ 两份文件内容完全对应，英文为中文的翻译
7. 更新 log.md
